...
def create_black_hole_effect(frame, time_elapsed):
    """ สร้าง Swirl + หลุมดำหมุน + เปลี่ยนเป็นขาวดำ """
    h, w, _ = frame.shape
    ...
    # ✅ หมุนหลุมดำ + Star Dust
    rotation_speed = 90  # ปรับความเร็วการหมุน
    angle = (time_elapsed * rotation_speed) % 360  # หมุนตามเวลา
    M = cv2.getRotationMatrix2D((w//2, h//2), angle, 1.0)
    black_hole_layer = cv2.warpAffine(black_hole_layer, M, (w, h))

    # ✅ รวม Swirl + หลุมดำที่หมุนได้
...
def process_frame(frame):
    """
    ถ้าตรวจจับโทรศัพท์ → เปิด Swirl Effect + หลุมดำหมุน
    ...
    # ✅ ถ้าตรวจจับโทรศัพท์ → เริ่มเอฟเฟกต์ทันที
    if len(phones) > 0:
        black_hole_active = True
        black_hole_start_time = current_time

    # ✅ ถ้าเอฟเฟกต์เปิดอยู่ ให้คงอยู่ต่อ 10 วินาที
    if black_hole_active:
        ...
        if elapsed_time < black_hole_duration:
            ...
            frame = cv2.addWeighted(create_black_hole_effect(frame, elapsed_time), fade_factor, frame, 1.0 - fade_factor, 0)
