    ...
    for y in range(h):
        for x in range(w):
            ...
            if r < radius:
                ...
                angle = math.atan2(dy, dx) + theta + angle_offset  # เพิ่ม angle_offset เพื่อให้เอฟเฟกต์หมุน
...
def create_black_hole_effect(frame, time_elapsed):
    """สร้าง Swirl + เปลี่ยนเป็นขาวดำ + ให้เอฟเฟกต์ Swirl หมุน"""
    h, w, _ = frame.shape
    ...
    # คำนวณมุมหมุนของเอฟเฟกต์ Swirl
    rotation_speed = 0.5  # ปรับความเร็วการหมุน
    angle_offset = time_elapsed * rotation_speed  # มุมที่เพิ่มขึ้นตามเวลา

    # สร้าง Swirl Effect โดยให้มุมหมุนตามเวลา
    swirl_frame = swirl_effect(frame_gray, center=(w // 2, h // 2), radius=250, strength=3.0, angle_offset=angle_offset)

    return swirl_frame

def process_frame(frame):
    """
    ถ้าตรวจจับโทรศัพท์ → เปิดเอฟเฟกต์ Swirl + หมุน
    ...
    # ถ้าตรวจจับโทรศัพท์และเอฟเฟกต์ยังไม่ active ให้เริ่มเอฟเฟกต์และตั้งเวลาเพียงครั้งเดียว
    if len(phones) > 0:
        if not black_hole_active:
            black_hole_active = True
            black_hole_start_time = current_time

    # ถ้าเอฟเฟกต์ active อยู่
...
def main():
    ...
    while cap.isOpened():
        ...
        cv2.imshow("Swirling Effect", frame)
